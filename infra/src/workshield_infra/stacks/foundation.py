...
from aws_cdk import CfnDeletionPolicy, CfnOutput, CfnTag, RemovalPolicy, Stack
    ...
    def __init__(self, scope: Construct, construct_id: str, *, config: DeploymentConfig, **kwargs: object) -> None:
        ...
        self.instance_role.add_to_policy(
            iam.PolicyStatement(
                ...
                resources=[f"arn:{self.partition}:secretsmanager:{self.region}:{self.account}:secret:/workshield/{config.environment}/*"],
            )
        )
        self.instance_role.add_to_policy(
            iam.PolicyStatement(
                actions=["ssm:GetParameter", "ssm:GetParameters", "ssm:PutParameter"],
                resources=[f"arn:{self.partition}:ssm:{self.region}:{self.account}:parameter/workshield/{config.environment}/*"],
            )
        )
        self.instance_role.add_to_policy(
            iam.PolicyStatement(
                ...
                resources=[
                    f"arn:{self.partition}:logs:{self.region}:{self.account}:log-group:/workshield/{config.environment}/application:*"
                ],
            )
        )
        ...
        self.log_group = logs.LogGroup(
            self,
            "ApplicationLogGroup",
            log_group_name=f"/workshield/{config.environment}/application",
        ...
        for name, value in {
            "release/active-tag": "__UNSET__",
            "release/active-api-image": "__UNSET__",
            "release/active-mcp-image": "__UNSET__",
            "runtime/binding": "{}",
            ...
            ssm.StringParameter(
                self,
                f"Parameter{name.title().replace('/', '').replace('-', '')}",
                parameter_name=f"/workshield/{config.environment}/{name}",
                string_value=value,
            )
