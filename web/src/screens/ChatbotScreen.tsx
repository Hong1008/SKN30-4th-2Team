...
export default function ChatbotScreen({ reviewId, focusClauseId, focusClauseName, focusClauseTitle, focusClauseStatus, focusClauseCategory, onClose, onClearFocus, onReviewUnavailable, isOpen }: Props) {
  ...
  return <aside className={`fixed inset-y-0 right-0 z-50 flex w-full max-w-[480px] flex-col border-l border-slate-200 bg-white shadow-2xl transition-transform sm:w-[440px] ${isOpen ? 'translate-x-0' : 'translate-x-full pointer-events-none'}`} role="dialog" aria-modal="false" aria-hidden={!isOpen} aria-labelledby="chat-panel-title">
    ...
    <div className="flex-1 space-y-4 overflow-y-auto p-4" aria-label="챗봇 대화 내용" aria-live="polite">
      ...
      {isSending && <div className="flex justify-start" role="status" aria-live="polite"><div className="rounded-xl border border-slate-200 bg-slate-50 px-3 py-2.5 text-sm text-slate-600"><span className="sr-only">답변을 작성하고 있습니다</span><span aria-hidden="true" className="inline-flex items-end gap-1 py-1"><span data-testid="typing-dot" className="size-1.5 animate-bounce rounded-full bg-slate-400 [animation-delay:-0.3s]" /><span data-testid="typing-dot" className="size-1.5 animate-bounce rounded-full bg-slate-400 [animation-delay:-0.15s]" /><span data-testid="typing-dot" className="size-1.5 animate-bounce rounded-full bg-slate-400" /></span></div></div>}
      <div ref={bottomRef} />
    </div>
