...
from app.chat.schemas import ChatOutcome
from app.grounding.schemas import GroundingStatus
from app.review_sessions.domain import ScopeStatus, SelectionSource
from app.suggestions.schemas import SuggestionOutcome
...
class MetadataCode(BaseModel):
    ...
    enabled_for_mvp: bool | None = Field(default=None, description="MVP 선택 활성화 여부")
...
class CategoryMetadata(BaseModel):
    ...
    anchors: list[str] = Field(default_factory=list, description="카테고리 앵커 키워드 목록")
...
class FilePolicy(BaseModel):
    ...
    single_file_only: bool = Field(default=True, description="단일 파일 업로드 전용 여부")
    encrypted_file_allowed: bool = Field(default=False, description="암호화 파일 허용 여부")


class FeatureFlags(BaseModel):
    chat: bool = Field(default=True, description="질의응답(챗봇) 기능 활성화 여부")
    basic_suggestion: bool = Field(default=True, description="기본 협의 문구 제안 기능 활성화 여부")
    confidence_score: bool = Field(default=False, description="신뢰도 점수 노출 여부")
    suggestion_edit: bool = Field(default=False, description="제안 문구 편집/임시저장 기능 활성화 여부")
    single_clause_rereview: bool = Field(default=False, description="단일 조항 재검토 기능 활성화 여부")
    server_side_cancel: bool = Field(default=True, description="서버 사이드 검토 취소/폐기 기능 활성화 여부")
...
class MetadataResponse(BaseModel):
    ...
    toxic_patterns: list[ToxicPatternMetadata] = Field(description="주의/독소 패턴 목록")
    ...
    selection_sources: list[SelectionSource] = Field(description="계약 유형 선택 경로 목록")
