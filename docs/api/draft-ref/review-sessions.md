...
업로드 계약서 자체는 OpenAI에 전송하지 않는다. OpenAI는 채팅 단계에서 현재 사용자 질문의 앞 80자 분류에만 사용하며, 최종 답변은 자체 호스팅 vLLM `RedHatAI/Qwen3.5-9B-FP8-dynamic`이 생성한다. RunPod를 활용하는 운영 구성에서는 조항 검색·재정렬을 위해 계약 조항과 후보 표준조항이 전송될 수 있으므로, 이 경로는 OpenAI 질문 분류와 분리해 고지한다.
