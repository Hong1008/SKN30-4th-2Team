...
* 관련 문서:
  ...
  * [Suggestions 출처 선택과 백엔드 결합](./0727-suggestions-source-binding.md)
...
~~모델 선정 전 Gemma 4 31B IT, Qwen 3.6 27B Dense,
Qwen 3.6 35B-A3B를 공통 vLLM 또는 SGLang의 같은 BF16 조건에서 먼저
비교하고, 통과 후보를 같은 양자화 방식으로 다시 평가한다.~~

부트캠프 MVP의 일정·예산과 제한된 LLM 역할을 고려해 모델 검증 순서를
다음과 같이 결정한다.

* 우선 운영 후보: `hf.co/unsloth/Qwen3.5-9B-GGUF:Q4_K_M`
* 우선 후보 양자화: Q4_K_M
* 목표 운영 GPU: VRAM 24GB급 이하
* 평가 서빙 후보: vLLM OpenAI-compatible server
* 초기 동시 처리 수: 1
* 기본 생성 모드: 비추론 모드
* 최대 출력: Suggestions 기준 512 토큰, 상한 1,000 토큰
* 외부 LLM 자동 폴백: 사용하지 않음

Qwen3.5 9B를 고정 fixture 20개에 최소 3회씩 먼저 평가한다. 구조화 출력,
출처 선택, 허위 수치, 법률적 단정과 문구 품질 hard gate를 통과하면
27B~35B 대형 모델 비교를 생략하고 운영 적합성 검증으로 진행한다.

9B가 기준을 충족하지 못한 경우에만 Gemma 4 31B IT, Qwen 3.6 27B
Dense, Qwen 3.6 35B-A3B를 같은 엔진·정밀도·프롬프트·JSON Schema에서
확장 비교한다.

vLLM은 평가 서빙 후보일 뿐 아직 호환성이 승인되지 않았다. 정확한 모델
revision, 엔진 버전·image digest, API 계약, GPU와 컨텍스트 크기는
연결·구조화 출력 검증 후 고정한다. 운영 Pod·Serverless 경계는
`0727-runpod-shxt.md`를 따른다.

다음 테스트는 우선 후보의 정확한 모델·양자화로 수행한다.
...
* ~~Qwen3.5 9B Q4 계열을 다른 대형 모델과 함께 비교한 뒤 최종 모델과
  양자화를 결정한다.~~
  Qwen3.5 9B Q4_K_M을 먼저 평가하고, 통과하면 대형 모델 비교를 생략한다.
  정확한 대상 환경 검증 전에는 운영 승인 모델로 간주하지 않는다.
...
~~Gemma 4 31B IT, Qwen 3.6 27B Dense, Qwen 3.6 35B-A3B를 기본
비교 후보로 두고 같은 엔진·프롬프트·스키마에서 측정한다.~~

대형 모델은 Qwen3.5 9B가 안전성 또는 문구 품질 hard gate를 충족하지
못했을 때만 비교하는 확장 후보로 둔다. 품질 상한을 확인하기 위한 대형
모델 비교보다 MVP에 필요한 품질을 제한된 자원으로 충족하는지를 먼저
검증한다.
...
~~Gemma 4 31B IT, Qwen 3.6 27B Dense, Qwen 3.6 35B-A3B를 공통
로컬 서빙 경로에서 먼저 비교한다.~~

Qwen3.5 9B Q4_K_M을 우선 검증하고 합격 기준을 충족하면 운영 적합성
검증으로 진행한다. 기준을 충족하지 못할 때만 27B~35B 후보 비교로
확장한다. 정확한 모델·엔진·배포 구성을 고정하고 기능별 체크리스트를
통과하기 전까지 본 결정의 상태는 `Proposed`로 유지한다. 4B Ollama와
외부 Gemini 결과는 이전 ID 복사 계약의 참고 기록이며 운영 승인을
뜻하지 않는다.
